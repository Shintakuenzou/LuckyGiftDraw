...
export function Home() {
  ...
  useEffect(() => {
    fetch("https://api.github.com/users/Shintakuenzou")
    ...
    <div className="container">
      <header>
        <div>
          ...
          <span>
            Location: {user.location} - <a href={user.url} target="_blank">GitHub</a>
          </span>
        </div>
      </header>
      ...
      <input id="inputNameUser" type="text" placeholder="Enter a participant name" onChange={(callbackFuncao) => setUserName(callbackFuncao.target.value)} value={userName} />
      <div className="container-btns">
        ...
        >
          <UserSwitch size={24} /> Raffle user
        </button>
      </div>
      ...
      <div id="container-raffle-user">
        <div>
          <p>
            Qtd. Sorteios: <span id="counter">{count}</span>
          </p>
      ...
      <div className="list-users">
        <h2>
          List of user <span>({users.length})</span>
        </h2>
      ...
      {users.map((usuario) => (
        <div className="card" key={usuario.id}>
          <div>
            <p>
              Name: <strong>{usuario.name}</strong>
            </p>
            <p>Time: {usuario.time}</p>
          </div>
          <div>
            <a onClick={() => removeUser(usuario.id)} className="link-card" href="#">
